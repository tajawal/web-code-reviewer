...
*Note: This is an automated summary due to token limits. Full review requires manual inspection.*`;
  ...
  /**
   * Delete previous bot comments on the PR
   */
  async deletePreviousComments() {
    try {
      ...
      const botComments = comments.filter(comment => 
        comment.body.includes('## 🤖 DeepReview') || // Match the header
        comment.body.includes('*This review was automatically generated by @tajawal/web-code-review*') // Match the footer
      );

      for (const comment of botComments) {
        core.info(`🗑️ Deleting previous bot comment: ${comment.id}`);
      ...
      if (botComments.length > 0) {
        core.info(`✅ Deleted ${botComments.length} previous bot comment(s)`);
      }
    } catch (error) {
  ...
  async addPRComment(comment) {
    ...
    try {
      // First delete any previous bot comments
  ...
  getIssueUrl(issueNumber) {
    return `https://github.com/${this.context.repo.owner}/${this.context.repo.repo}/issues/${issueNumber}`;
  }

  /**
   * Generate PR comment content with enhanced JSON parsing and issue links
   */
  generatePRComment(shouldBlockMerge, changedFiles, llmResponse, criticalIssueMap = new Map()) {
    ...
    if (extractedData.issues.length > 0) {
      ...
      if (criticalIssues.length > 0) {
        ...
        criticalIssues.forEach(issue => {
          ...
          const issueNumber = criticalIssueMap.get(issue.originalId);
          issueDetails += `🔴 ${issue.originalId} - ${issue.category.toUpperCase()} (Chunk ${issue.chunk})${issueNumber ? ` - [View Issue #${issueNumber}](${this.getIssueUrl(issueNumber)})` : ''}\n`;
          if (issue.snippet) {
...
`;
  ...
  /**
   * Create GitHub issue for critical issue
   */
  async createIssueForCriticalIssue(issue, prNumber) {
    try {
      const issueTitle = `🚨 Critical: ${issue.category} in ${issue.file}`;
      
      // Create issue body with detailed information
      const issueBody = `## 🚨 Critical Issue Detected
      
**Category**: ${issue.category}
**File**: \`${issue.file}\`
**Lines**: ${issue.lines.join('-')}
**Severity Score**: ${issue.severity_score?.toFixed(1) || 'N/A'}/5.0
...
### 🔍 Code Context
\`\`\`${getLanguageForFile(issue.file)}
${issue.snippet || 'Code snippet not available'}
\`\`\`

### 💡 Suggested Fix
${issue.fix_summary || 'No specific fix suggestion available'}

${issue.fix_code_patch ? `\`\`\`${getLanguageForFile(issue.file)}
${issue.fix_code_patch}
\`\`\`` : ''}

### 🧪 Test Recommendation
${issue.tests || 'No specific test recommendations available'}

### 🔗 Related PR
This issue was detected in PR #${prNumber}

### 🎯 Risk Factors
${issue.risk_factors ? `- Impact: ${issue.risk_factors.impact}
- Exploitability: ${issue.risk_factors.exploitability}
- Likelihood: ${issue.risk_factors.likelihood}
- Blast Radius: ${issue.risk_factors.blast_radius}
- Evidence Strength: ${issue.risk_factors.evidence_strength}` : 'Risk factors not available'}

---
*This issue was automatically created by DeepReview based on AI code analysis.*`;
      ...
      const { data: newIssue } = await this.octokit.rest.issues.create({
        ...
        title: issueTitle,
        body: issueBody,
        labels: ['critical', 'security', 'needs-review', 'ai-detected']
      });

      core.info(`✅ Created issue #${newIssue.number} for critical issue: ${issue.category}`);
      return newIssue.number;
    } catch (error) {
      core.warning(`⚠️  Failed to create issue for critical issue: ${error.message}`);
      return null;
    }
  }
  ...
  async runReview() {
    ...
    if (this.logLLMResponse(llmResponse)) {
      ...
      // Track created issue numbers for each critical issue
      const criticalIssueMap = new Map(); // Map issue ID to GitHub issue number
      
      // Create issues for critical issues
      if (extractedData.issues.length > 0) {
        const criticalIssues = extractedData.issues.filter(i => 
          i.severity_proposed === 'critical' && i.confidence >= 0.6
        );
        
        if (criticalIssues.length > 0) {
          core.info(`📝 Creating GitHub issues for ${criticalIssues.length} critical issues...`);
          
          const prNumber = this.context.issue.number;
          const issuePromises = criticalIssues.map(async (issue) => {
            const issueNumber = await this.createIssueForCriticalIssue(issue, prNumber);
            if (issueNumber) {
              criticalIssueMap.set(issue.originalId, issueNumber);
            }
            return issueNumber;
          });
          
          const issueNumbers = await Promise.all(issuePromises);
          const validIssueNumbers = issueNumbers.filter(num => num !== null);
          
          if (validIssueNumbers.length > 0) {
            core.info(`✅ Created ${validIssueNumbers.length} GitHub issues: #${validIssueNumbers.join(', #')}`);
          }
        }
      }
      
      // Generate and post PR comment with issue links
      const prComment = this.generatePRComment(shouldBlockMerge, changedFiles, llmResponse, criticalIssueMap);
